...
void	 btree_apply_prefix(t_btree *root, void (*applyf)(void*))
{
	if (root == NULL)
		return ;
	applyf(root->item);
	btree_apply_prefix(root->left, ft_print_int_item);
	btree_apply_prefix(root->right, ft_print_int_item);
}
...
int	main(void)
{
	...
	root = ft_make_prefix(10, array);
	print_tree(root, 0);
	btree_apply_prefix(root, ft_print_int_item);
	free_tree(root);
	return (0);
}