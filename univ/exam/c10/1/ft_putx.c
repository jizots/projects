...
t_sfloat	*float_to_struct(float num, t_sfloat *bit_float)
{
	bit_float->sign = (uint8_t)num >> 31;
	bit_float->exp = (uint8_t)num << 1;
	bit_float->frac = (uint32_t)num << 9;
	return (bit_float);
}
